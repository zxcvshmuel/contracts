...
<header {{ $attributes->class([
    'filament-main-topbar sticky top-0 z-10 flex h-auto w-full shrink-0 items-center border-b bg-white',
    ...
    <div class="flex items-center w-full px-2 sm:px-4 md:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between flex-1">
            <x-filament::button
                @class(['ml-1 ctc color-blue'])
                @style(['background-color: blue'])
                x-init="init()"
                @click="$dispatch('color', { color: 'blue' })"
            >

            </x-filament::button>
            <x-filament::button @class(['ml-1'])  @style([
            'background-color: red'])
                                x-init="init()"
                @click="$dispatch('color', { color: 'red' })"
            >
            ...
            'background-color: green',
                                ...
                                x-init="init()"
                                @click="$dispatch('color', { color: 'green' })"
            >
            ...
            'background-color: orange',
                                ...
                                x-init="init()"
                                @click="$dispatch('color', { color: 'orange' })"
            >
